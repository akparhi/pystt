...
class Model(nn.Module):
    def __init__(self, flavor, device):
        ...
        size = self.bert.dim if hasattr(self.bert, 'dim') else self.bert.config.pooler_fc_size if hasattr(
            self.bert.config, 'pooler_fc_size') else self.bert.config.emb_dim if hasattr(self.bert.config, 'emb_dim') else self.bert.config.hidden_size
...
def compute_performance(config, model, loader):
    ...
    for x, y in loader:
        ...
        with torch.no_grad():
            ...
            loss1 = criterion(y_scores1.view(
                y1.size(0) * y1.size(1), -1), y1.view(y1.size(0) * y1.size(1)))
            loss2 = criterion(y_scores2.view(
                y2.size(0) * y2.size(1), -1), y2.view(y2.size(0) * y2.size(1)))
            loss = loss1 + loss2
    ...
    for label in range(0, 5):
        recall[label] = num_correct[label] / \
            num_ref[label] if num_ref[label] > 0 else 0
        precision[label] = num_correct[label] / \
            num_hyp[label] if num_hyp[label] > 0 else 0
        fscore[label] = (2 * recall[label] * precision[label] / (recall[label] +
                         precision[label])).item() if recall[label] + precision[label] > 0 else 0
...
def fit(config, model, checkpoint_path, train_loader, valid_loader, iterations, valid_period=200, lr=1e-5):
    ...
    while True:
        ...
        for x, y in tqdm(train_loader):
            x = x.long().to(device)
            y = y.long().to(device)
            drop_at_boundaries(config.dab_rate, x, y, config.cls_token_id,
                               config.sep_token_id, config.pad_token_id)
            y1 = y[:, :, 0]
            y2 = y[:, :, 1]
            optimizer.zero_grad()
            ...
            if iteration % valid_period == valid_period - 1:
                ...
                valid_loss, valid_accuracy_case, valid_accuracy_punc, valid_fscore = compute_performance(
                ...
                print(iteration + 1, train_loss, valid_loss,
...
def train(config, train_x_fn, train_y_fn, valid_x_fn, valid_y_fn, checkpoint_path):
    X_train, Y_train = batchify(config.max_length, torch.load(
        train_x_fn), torch.load(train_y_fn))
    X_valid, Y_valid = batchify(config.max_length, torch.load(
        valid_x_fn), torch.load(valid_y_fn))
    ...
    fit(config, model, checkpoint_path, train_loader,
        valid_loader, config.updates, config.period, config.lr)


def run_eval(config, test_x_fn, test_y_fn, checkpoint_path):
    X_test, Y_test = batchify(config.max_length, torch.load(
        test_x_fn), torch.load(test_y_fn))
...
def generate_predictions(config, checkpoint_path):
    loaded = torch.load(
        checkpoint_path, map_location=config.device if torch.cuda.is_available() else 'cpu')
    if 'config' in loaded:
        config = Config(**loaded['config'])
        init(config)
    ...
    for line in sys.stdin:
        ...
        if config.debug:
            print(line)
        tokens = [config.cls_token] + \
            config.tokenizer.tokenize(line) + [config.sep_token]
        if config.debug:
            print(tokens)
        previous_label = punctuation['PERIOD']
        first_time = True
                ...
                if config.lang == 'fr':
                    if token.endswith('</w>'):
                        ...
                        if was_word:
                            print(' ', end='')
                        print(cased_token +
                              punctuation_syms[punc_label], end='')
                        was_word = True
                    else:
            ...
            for i, char in enumerate(line):
                if char in mapped_punctuation:
                    if i > start and line[start: i].strip() != '':
                        process_segment(
                            line[start: i], mapped_punctuation[char])
                    start = i + 1
            if start < len(line):
                    ...
                    if token in mapped_punctuation:
                        if previous_token != None:
                            print(previous_token,
                                  mapped_punctuation[token], sep='\t')
                        previous_token = None
                    # remove non-alphanumeric tokens
                    elif not re.search('[\p{Han}\p{Ll}\p{Lu}\d]', token):
                        continue
                    else:
                        if previous_token != None:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--seed", help="random seed",
                        default=default_config.seed, type=int)
    parser.add_argument("--lang", help="language (fr, en, zh)",
                        default=default_config.lang, type=str)
    parser.add_argument("--flavor", help="bert flavor in transformers model zoo",
                        default=default_config.flavor, type=str)
    parser.add_argument("--max-length", help="maximum input length",
                        default=default_config.max_length, type=int)
    parser.add_argument("--batch-size", help="size of batches",
                        default=default_config.batch_size, type=int)
    parser.add_argument("--device", help="computation device (cuda, cpu)",
                        default=default_config.device, type=str)
    parser.add_argument("--debug", help="whether to output more debug info",
                        default=default_config.debug, type=bool)
    parser.add_argument("--updates", help="number of training updates to perform",
                        default=default_config.updates, type=bool)
    parser.add_argument("--period", help="validation period in updates",
                        default=default_config.period, type=bool)
    parser.add_argument("--lr", help="learning rate",
                        default=default_config.lr, type=bool)
    parser.add_argument("--dab-rate", help="drop at boundaries rate",
                        default=default_config.dab_rate, type=bool)
